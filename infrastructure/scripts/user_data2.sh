#!/bin/bash

# Функция для логирования
function log() {
    sep="----------------------------------------------------------"
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] $sep " >> /home/ubuntu/user_data_execution.log
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] [INFO] $1" >> /home/ubuntu/user_data_execution.log
}

# Создаем файл логов, если он не существует
touch /home/ubuntu/user_data_execution.log
chown ubuntu:ubuntu /home/ubuntu/user_data_execution.log
chmod 644 /home/ubuntu/user_data_execution.log

log "Starting user data script execution"

# Устанавливаем yc CLI
log "Installing yc CLI"
export HOME="/home/ubuntu"
if curl https://storage.yandexcloud.net/yandexcloud-yc/install.sh | bash; then
    log "yc CLI installed successfully"
else
    log "Failed to install yc CLI"
    exit 1
fi

# Изменяем владельца директории yandex-cloud и её содержимого
log "Changing ownership of yandex-cloud directory"
if sudo chown -R ubuntu:ubuntu $HOME/yandex-cloud; then
    log "Ownership changed successfully"
else
    log "Failed to change ownership"
    exit 1
fi

# Применяем изменения из .bashrc
log "Applying changes from .bashrc"
if [ -f /home/ubuntu/.bashrc ]; then
    source /home/ubuntu/.bashrc
    log ".bashrc sourced successfully"
else
    log "File /home/ubuntu/.bashrc does not exist"
    exit 1
fi

# Проверяем, что yc доступен
if command -v yc &> /dev/null; then
    log "yc CLI is now available"
    yc --version >> /home/ubuntu/user_data_execution.log
else
    log "yc CLI is still not available. Adding it to PATH manually"
    export PATH="$PATH:$HOME/yandex-cloud/bin"
    yc --version >> /home/ubuntu/user_data_execution.log
fi

# Настраиваем yc CLI
log "Configuring yc CLI"
if yc config set token ${token} && \
   yc config set cloud-id ${cloud_id} && \
   yc config set folder-id ${folder_id}; then
    log "yc CLI configured successfully"
else
    log "Failed to configure yc CLI"
    exit 1
fi

# Устанавливаем jq
log "Installing jq"
if sudo apt-get update && sudo apt-get install -y jq; then
    log "jq installed successfully"
else
    log "Failed to install jq"
    exit 1
fi

# Получаем ID мастер-ноды Dataproc кластера
log "Getting Dataproc master node ID"
DATAPROC_MASTER_FQDN=$(yc compute instance list --format json | jq -r '.[] | select(.labels.subcluster_role == "masternode") | .fqdn')

if [ -z "$DATAPROC_MASTER_FQDN" ]; then
    log "Failed to get master node ID"
    exit 1
fi

log "Master node FQDN: $DATAPROC_MASTER_FQDN"

# Создаем директорию .ssh и настраиваем приватный ключ
log "Creating .ssh directory and setting up private key"
mkdir -p /home/ubuntu/.ssh
echo "Private Key: ${private_key}" >> /home/ubuntu/user_data_execution.log
echo "${private_key}" > /home/ubuntu/.ssh/dataproc_key
chmod 600 /home/ubuntu/.ssh/dataproc_key
chown ubuntu:ubuntu /home/ubuntu/.ssh/dataproc_key

# Добавляем конфигурацию SSH для удобного подключения к мастер-ноде
log "Adding SSH configuration for master node connection"
cat <<EOF > /home/ubuntu/.ssh/config
Host dataproc-master
    HostName $DATAPROC_MASTER_FQDN
    User ubuntu
    IdentityFile ~/.ssh/dataproc_key
    StrictHostKeyChecking no
    UserKnownHostsFile /dev/null
EOF

chown ubuntu:ubuntu /home/ubuntu/.ssh/config
chmod 600 /home/ubuntu/.ssh/config

# Настраиваем SSH-agent
log "Configuring SSH-agent"
eval $(ssh-agent -s)
echo "eval \$(ssh-agent -s)" >> /home/ubuntu/.bashrc
ssh-add /home/ubuntu/.ssh/dataproc_key
echo "ssh-add /home/ubuntu/.ssh/dataproc_key" >> /home/ubuntu/.bashrc

# Устанавливаем дополнительные полезные инструменты
log "Installing additional tools"
if sudo apt-get update && sudo apt-get install -y tmux htop iotop; then
    log "Additional tools installed successfully"
else
    log "Failed to install additional tools"
    exit 1
fi

# Устанавливаем s3cmd
log "Installing s3cmd"
if sudo apt-get install -y s3cmd; then
    log "s3cmd installed successfully"
else
    log "Failed to install s3cmd"
    exit 1
fi

# Настраиваем s3cmd
log "Configuring s3cmd"

cat <<EOF > /home/ubuntu/.s3cfg
[default]
access_key = ${access_key}
secret_key = ${secret_key}
host_base = storage.yandexcloud.net
host_bucket = %(bucket)s.storage.yandexcloud.net
use_https = True
EOF

chown ubuntu:ubuntu /home/ubuntu/.s3cfg
chmod 600 /home/ubuntu/.s3cfg

# Определяем целевой бакет
TARGET_BUCKET=${s3_bucket}

# Определяем исходную и целевую директории
SOURCE_DIR="s3://otus-mlops-source/"
TARGET_DIR="s3://$TARGET_BUCKET/"

# Проверяем, что исходная директория существует и содержит файлы
log "Checking if source directory exists and contains files"
if s3cmd ls --config=/home/ubuntu/.s3cfg "$SOURCE_DIR" | grep -q "DIR"; then
    log "Source directory exists and contains files"
else
    log "Source directory is empty or does not exist"
    exit 1
fi

# Копируем все файлы из исходного бакета в целевой бакет
log "Copying all files from source bucket to destination bucket"
if s3cmd cp --config=/home/ubuntu/.s3cfg --acl-public --recursive "$SOURCE_DIR*" "$TARGET_DIR"; then
    log "All files successfully copied to $TARGET_DIR"
    log "Listing contents of $TARGET_BUCKET"
    s3cmd ls --config=/home/ubuntu/.s3cfg "$TARGET_DIR" >> /home/ubuntu/user_data_execution.log
else
    log "Error occurred while copying files"
    exit 1
fi

# Создаем директорию для скриптов на прокси-машине
log "Creating scripts directory on proxy machine"
mkdir -p /home/ubuntu/scripts

# Копируем скрипт upload_data_to_hdfs.sh на прокси-машину
log "Copying upload_data_to_hdfs.sh script to proxy machine"
echo "${upload_data_to_hdfs_content}" > /home/ubuntu/scripts/upload_data_to_hdfs.sh

# Заменяем плейсхолдер на значение переменной TARGET_BUCKET
if [ -z "$TARGET_BUCKET" ]; then
    log "TARGET_BUCKET is not set"
    exit 1
fi
sed -i "s/{{ s3_bucket }}/$TARGET_BUCKET/g" /home/ubuntu/scripts/upload_data_to_hdfs.sh

# Устанавливаем правильные разрешения для скрипта на прокси-машине
log "Setting permissions for upload_data_to_hdfs.sh on proxy machine"
chmod +x /home/ubuntu/scripts/upload_data_to_hdfs.sh

# Проверяем подключение к мастер-ноде
log "Checking connection to master node"
source /home/ubuntu/.bashrc
if ssh -i /home/ubuntu/.ssh/dataproc_key -o StrictHostKeyChecking=no ubuntu@$DATAPROC_MASTER_FQDN "echo 'Connection successful'"; then
    log "Connection to master node successful"
else
    log "Failed to connect to master node"
    exit 1
fi

# Копируем скрипт upload_data_to_hdfs.sh с прокси-машины на мастер-ноду
log "Copying upload_data_to_hdfs.sh script from proxy machine to master node"
if scp -i /home/ubuntu/.ssh/dataproc_key -o StrictHostKeyChecking=no /home/ubuntu/scripts/upload_data_to_hdfs.sh ubuntu@$DATAPROC_MASTER_FQDN:/home/ubuntu/; then
    log "Script copied to master node successfully"
else
    log "Failed to copy script to master node"
    exit 1
fi

# Устанавливаем правильные разрешения для скрипта на мастер-ноде
log "Setting permissions for upload_data_to_hdfs.sh on master node"
if ssh -i /home/ubuntu/.ssh/dataproc_key -o StrictHostKeyChecking=no ubuntu@$DATAPROC_MASTER_FQDN "chmod +x /home/ubuntu/upload_data_to_hdfs.sh"; then
    log "Permissions set on master node successfully"
else
    log "Failed to set permissions on master node"
    exit 1
fi

log "Script upload_data_to_hdfs.sh has been copied to the master node"

# Изменяем владельца лог-файла
log "Changing ownership of log file"
sudo chown ubuntu:ubuntu /home/ubuntu/user_data_execution.log

log "User data script execution completed"