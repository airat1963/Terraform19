...
function log() {
    sep="----------------------------------------------------------"
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] $sep " | tee -a /home/ubuntu/user_data_execution.log
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] [INFO] $1" | tee -a /home/ubuntu/user_data_execution.log
}
...
if [ $? -eq 0 ]; then
    ...
    s3cmd ls --config=/home/ubuntu/.s3cfg "$TARGET_DIR" | tee -a /home/ubuntu/user_data_execution.log
else
