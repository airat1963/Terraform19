...
resource "yandex_compute_instance" "proxy" {
  ...
  metadata = {
    ...
    user-data = templatefile("${path.root}/scripts/user_data1.sh", {
      token                       = var.yc_token
      ...
      dataproc_key                = file(var.private_key_path)
    })
    //file_hash = md5(file("${path.root}/scripts/upload_data_to_hdfs.sh")) # Принудительное обновление
  }
  ...
  provisioner "remote-exec" {
    //timeout="10m"   # more time ? if it so small
    inline = [
      " echo 'Hello World'",
      "sleep 30" ,    # small delay
